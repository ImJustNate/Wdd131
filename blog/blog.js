...
function appendBookCards(item){
  ...
  newSection.className = "bookcard";
  newSection.id = item.id;
  newSection.innerHTML = `
            <img src="${item.imgSrc} " alt="${item.imgAlt} '">
            <div class="bookreveiw">  
                <h2><a href="#">${item.title}</a></h2>
                <p><i>Age of prose:</i> ${item.ages}</p>
                <p><i>Type of plot: </i>${item.genre}</p>
                <p><i>Perspective: </i>${item.description} (${item.date})
...
articles.forEach(appendBookCards)
